...
def loads_cities(area_map):
    ...
    with open('crawler/area.txt', encoding='UTF-8') as f:
        cont = f.readline()
    areas = []
    ...
    for province in data:
        ...
        if province_name in alias:
            province_name = alias.get(province_name)
        if province_name == '浙江' or province_name == '台湾':
            continue
        if province_name == '吉林':
            area_id = 220000
        else:
            area_id = area_map[province_name]
        province_arr.append(province_name+"|"+str(area_id))
        areas.append(
            {
                "name":province_name,
                "type":"province",
    ...
    province_str = ",".join(province_arr)
    print(province_str)

        # cities = []
        # if hasattr(province, "City"):
...
res = format_str(area_str)
all = area_str.split(',')
data = []
for item in all:
    if item in province_str:
        continue
    else:
        data.append(item)
print(",".join(data))
# print(loads_cities(res))